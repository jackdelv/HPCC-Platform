...
write_rec(dataset(simpleRec) sd) := EMBED(parquet: option('write'), MaxRowSize(2), destination('/datadrive/dev/test_data/simple.parquet'))
ENDEMBED;

DATASET(simpleRec) read_rec() := EMBED(parquet: option('read'), location('/datadrive/dev/test_data/simple.parquet'))
ENDEMBED;
